"""运行时对象（WS 管理器与事件循环槽位）（Phase 1.2 自 main.py 机械搬移，未做语义改动）。"""

import json
from typing import List, Dict, Any
from fastapi import WebSocket
from app.domain.canvas_task_service import task_event
...
class ConnectionManager:
    ...
    async def broadcast_canvas_task_updated(self, task: Dict[str, Any]):
        data = json.dumps({"type": "canvas_task_updated", "task": task_event(task)})
        for connection in self.active_connections[:]:
            try:
                await connection.send_text(data)
            except Exception as e:
                print(f"Broadcast canvas task error: {e}")
                self.active_connections.remove(connection)
